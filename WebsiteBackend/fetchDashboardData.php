...
$orderCounts = [
    ...
    'Ongoing' => 0,
    'Completed' => 0,
    'Cancelled' => 0 // This already handles cancelled orders
];
...
// Assuming delivery_date is stored as 'YYYY-MM-DD'
$sqlSales = "SELECT MONTH(delivery_date) as month, 
...
// Prepare SQL to get sales trends (sum of sales grouped by month)
$sqlSales2 = "SELECT MONTH(delivery_date) as month, 
                    SUM(total_price) as total_sales 
             FROM orders 
             WHERE station_id = ? AND status = 'Cancelled' 
             GROUP BY MONTH(delivery_date) 
             ORDER BY MONTH(delivery_date)";

$stmtSales2 = $conn->prepare($sqlSales2);
$stmtSales2->bind_param('s', $station_id); // Bind station_id
$stmtSales2->execute();
$resultSales2 = $stmtSales2->get_result();

$salesTrends2 = array_fill(0, 12, 0); // Initialize array for 12 months (Jan-Dec)

// Map sales data to the correct month
while ($row = $resultSales2->fetch_assoc()) {
    $month2 = (int)$row['month']; // Get month as integer (1 = Jan, 2 = Feb, etc.)
    $total_sales2 = (float)$row['total_sales'];
    
    // Store total sales in the correct month index (month - 1 for zero-based index)
    $salesTrends2[$month2 - 1] = $total_sales2;
}
